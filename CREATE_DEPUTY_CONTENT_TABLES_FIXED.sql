-- ============================================
-- CREATE DEPUTY CONTENT TABLES (FIXED)
-- ============================================
...
DO $$ 
BEGIN
  -- Electoral Programs foreign key
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'deputy_electoral_programs_deputy_id_fkey'
  ) THEN
    ALTER TABLE public.deputy_electoral_programs
      ADD CONSTRAINT deputy_electoral_programs_deputy_id_fkey
      FOREIGN KEY (deputy_id) REFERENCES public.deputy_profiles(id) ON DELETE CASCADE;
  END IF;

  -- Achievements foreign key
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'deputy_achievements_deputy_id_fkey'
  ) THEN
    ALTER TABLE public.deputy_achievements
      ADD CONSTRAINT deputy_achievements_deputy_id_fkey
      FOREIGN KEY (deputy_id) REFERENCES public.deputy_profiles(id) ON DELETE CASCADE;
  END IF;
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'deputy_events_deputy_id_fkey'
  ) THEN
    ALTER TABLE public.deputy_events
      ADD CONSTRAINT deputy_events_deputy_id_fkey
      FOREIGN KEY (deputy_id) REFERENCES public.deputy_profiles(id) ON DELETE CASCADE;
  END IF;
END $$;
...
BEGIN
  ...
  RAISE NOTICE '1. Verify tables: SELECT table_name FROM information_schema.tables WHERE table_name LIKE ''deputy_%'';';
  RAISE NOTICE '2. Check structure: SELECT column_name, data_type FROM information_schema.columns WHERE table_name = ''deputy_electoral_programs'';';
END $$;
