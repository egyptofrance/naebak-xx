-- ============================================================================
-- Migration: Add Upvoting System for Public Complaints
-- Date: 2025-11-01
...
-- Add comment
COMMENT ON TABLE complaint_votes IS 'Stores upvotes for public complaints. Each user/IP can vote once per complaint.';


-- 2. Add votes_count column to public_complaints
-- ============================================================================
...
CREATE INDEX IF NOT EXISTS idx_public_complaints_votes_count ON public_complaints(votes_count DESC);

-- Update existing complaints to have votes_count = 0
UPDATE public_complaints SET votes_count = 0 WHERE votes_count IS NULL;

-- Add comment
COMMENT ON COLUMN public_complaints.votes_count IS 'Cached count of upvotes for this complaint. Updated automatically by trigger.';
...
-- Add comment
COMMENT ON FUNCTION update_complaint_votes_count() IS 'Automatically updates votes_count in public_complaints when votes are added or removed.';
...
-- Grant access to anonymous users (for viewing only)
...
BEGIN
  ...
  RAISE NOTICE '✅ Columns added: public_complaints.votes_count';
  ...
  RAISE NOTICE '2. Check votes_count: SELECT id, title, votes_count FROM public_complaints LIMIT 5;';
