...
BEGIN
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "Users can view own applications"
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'job_applications' 
    AND policyname = 'Companies can view applications for their jobs'
  ) THEN
    EXECUTE 'CREATE POLICY "Companies can view applications for their jobs"
      ON public.job_applications FOR SELECT
      USING (
        job_id IN (
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'job_applications' 
    AND policyname = 'Companies can update applications for their jobs'
  ) THEN
    EXECUTE 'CREATE POLICY "Companies can update applications for their jobs"
